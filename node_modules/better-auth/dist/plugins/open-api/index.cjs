...
require('kysely');
require('../../shared/better-auth.DdZ-Id7n.cjs');
...
function getRequestBody(options) {
  ...
  if (options.body instanceof z__namespace.ZodObject || options.body instanceof z__namespace.ZodOptional) {
    ...
    return {
      required: options.body instanceof z__namespace.ZodOptional ? false : options.body ? true : false,
      content: {
        "application/json": {
          schema: {
            type: "object",
...
async function generator(ctx, options) {
  const baseEndpoints = api_index.getEndpoints(ctx, {
    ...options,
    plugins: []
  });
  ...
  for (const plugin of options.plugins || []) {
    ...
    const pluginEndpoints = api_index.getEndpoints(ctx, {
      ...options,
      plugins: [plugin]
    });
...
const openAPI = (options) => {
  ...
  return {
    id: "open-api",
    endpoints: {
      generateOpenAPISchema: session.createAuthEndpoint(
      ...
      openAPIReference: session.createAuthEndpoint(
        path,
        {
          method: "GET",
          metadata: {
