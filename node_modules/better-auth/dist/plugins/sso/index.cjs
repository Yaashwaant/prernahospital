...
const sso = (options) => {
  return {
    ...
    endpoints: {
       ...
       */
      createOIDCProvider: session.createAuthEndpoint(
        "/sso/register",
        {
          method: "POST",
       ...
       */
      signInSSO: session.createAuthEndpoint(
        "/sign-in/sso",
        {
          method: "POST",
          body: z__namespace.object({
        ...
        async (ctx) => {
          ...
          const state = await socialProviders_index.generateState(ctx);
          const redirectURI = `${ctx.context.baseURL}/sso/callback/${provider.providerId}`;
          const authorizationURL = await socialProviders_index.createAuthorizationURL({
            id: provider.issuer,
            options: {
      ...
      callbackSSO: session.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const stateData = await socialProviders_index.parseState(ctx);
          if (!stateData) {
          ...
          const tokenResponse = await socialProviders_index.validateAuthorizationCode({
            code,
          ...
          if (tokenResponse.idToken) {
            ...
            const verified = await socialProviders_index.validateToken(
          ...
          const linked = await socialProviders_index.handleOAuthUserInfo(ctx, {
            userInfo: {
            ...
            account: {
              ...
              accessToken: await socialProviders_index.setTokenUtil(
                tokenResponse.accessToken,
                ctx.context
              ),
              refreshToken: await socialProviders_index.setTokenUtil(
