...
require('./better-auth.DdZ-Id7n.cjs');
...
const schema = require('./better-auth.BSvTlywO.cjs');
const cookies_index = require('./better-auth.CXM9s0dx.cjs');
const hasPermission = require('./better-auth.yamXE7-N.cjs');
...
const createInvitation = (option) => {
  ...
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const canInvite = await hasPermission.hasPermission(
        {
          role: member.role,
      ...
      if (alreadyInvited.length && ctx.body.resend) {
        ...
        const defaultExpiration = 60 * 60 * 48;
        const newExpiresAt = date.getDate(
...
const acceptInvitation = (options) => session.createAuthEndpoint(
...
const rejectInvitation = (options) => session.createAuthEndpoint(
...
const cancelInvitation = (options) => session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const canCancel = await hasPermission.hasPermission(
      {
        role: member.role,
        options: ctx.context.orgOptions,
        permissions: {
...
const getInvitation = (options) => session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    if (invitation.email.toLowerCase() !== session$1.user.email.toLowerCase()) {
      throw new betterCall.APIError("FORBIDDEN", {
        message: ORGANIZATION_ERROR_CODES.YOU_ARE_NOT_THE_RECIPIENT_OF_THE_INVITATION
      });
    }
...
const listInvitations = (options) => session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const orgId = ctx.query?.organizationId || session$1.session.activeOrganizationId;
    if (!orgId) {
      throw new betterCall.APIError("BAD_REQUEST", {
...
const listUserInvitations = (options) => session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const userEmail = session$1?.user.email || ctx.query?.email;
    if (!userEmail) {
      throw new betterCall.APIError("BAD_REQUEST", {
...
const addMember = (option) => {
  ...
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      const session$1 = ctx.body.userId ? await session.getSessionFromCtx(ctx).catch((e) => null) : null;
      const orgId = ctx.body.organizationId || session$1?.session.activeOrganizationId;
      if (!orgId) {
        return ctx.json(null, {
          status: 400,
          body: {
...
const removeMember = (options) => session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const canDeleteMember = await hasPermission.hasPermission(
      {
        role: member.role,
        options: ctx.context.orgOptions,
        permissions: {
...
const updateMemberRole = (option) => session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    const canUpdateMember = await hasPermission.hasPermission(
      {
        role: member.role,
        options: ctx.context.orgOptions,
        permissions: {
...
const getActiveMember = (options) => session.createAuthEndpoint(
...
const leaveOrganization = (options) => session.createAuthEndpoint(
  "/organization/leave",
  {
    method: "POST",
    body: z__namespace.object({
...
const listMembers = (options) => session.createAuthEndpoint(
...
const getActiveMemberRole = (options) => session.createAuthEndpoint(
...
const createOrganization = (options) => {
  ...
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      const session$1 = await session.getSessionFromCtx(ctx);
      if (!session$1 && (ctx.request || ctx.headers)) {
        throw new betterCall.APIError("UNAUTHORIZED");
      }
      let user = session$1?.user || null;
      if (!user) {
        if (!ctx.body.userId) {
          throw new betterCall.APIError("UNAUTHORIZED");
        }
...
const checkOrganizationSlug = (options) => session.createAuthEndpoint(
...
const updateOrganization = (options) => {
  ...
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const canUpdateOrg = await hasPermission.hasPermission(
        {
          permissions: {
            organization: ["update"]
          },
...
const deleteOrganization = (options) => {
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const session = await ctx.context.getSession(ctx);
      if (!session) {
        throw new betterCall.APIError("UNAUTHORIZED", { status: 401 });
      }
      ...
      const canDeleteOrg = await hasPermission.hasPermission(
        {
          role: member.role,
          permissions: {
            organization: ["delete"]
...
const getFullOrganization = (options) => session.createAuthEndpoint(
...
const listOrganizations = (options) => session.createAuthEndpoint(
  "/organization/list",
  {
    method: "GET",
    use: [schema.orgMiddleware, schema.orgSessionMiddleware],
...
const createTeam = (options) => {
  ...
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      const session$1 = await session.getSessionFromCtx(ctx);
      const organizationId = ctx.body.organizationId || session$1?.session.activeOrganizationId;
      if (!session$1 && (ctx.request || ctx.headers)) {
        throw new betterCall.APIError("UNAUTHORIZED");
      }
      if (!organizationId) {
      ...
      if (session$1) {
        ...
        const canCreate = await hasPermission.hasPermission(
          {
            role: member.role,
            options: ctx.context.orgOptions,
            permissions: {
...
const removeTeam = (options) => session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    if (session$1) {
      ...
      if (!member || session$1.session?.activeTeamId === ctx.body.teamId) {
        throw new betterCall.APIError("FORBIDDEN", {
          message: ORGANIZATION_ERROR_CODES.YOU_ARE_NOT_ALLOWED_TO_DELETE_THIS_TEAM
        });
      }
      const canRemove = await hasPermission.hasPermission(
        {
          role: member.role,
          options: ctx.context.orgOptions,
          permissions: {
...
const updateTeam = (options) => {
  ...
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const canUpdate = await hasPermission.hasPermission(
        {
          role: member.role,
          options: ctx.context.orgOptions,
          permissions: {
...
const listOrganizationTeams = (options) => session.createAuthEndpoint(
...
const setActiveTeam = (options) => session.createAuthEndpoint(
...
const listUserTeams = (options) => session.createAuthEndpoint(
...
const listTeamMembers = (options) => session.createAuthEndpoint(
...
const addTeamMember = (options) => session.createAuthEndpoint(
...
const removeTeamMember = (options) => session.createAuthEndpoint(
...
Please refer to the documentation here: https://better-auth.com/docs/plugins/organization#dynamic-access-control`
      ...
      const canCreateRole = await hasPermission.hasPermission(
        {
          options,
          organizationId,
          permissions: {
  ...
  return session.createAuthEndpoint(
    "/organization/delete-role",
    {
      ...
      body: z__namespace.object({
        ...
        z__namespace.union([
          z__namespace.object({
            roleName: z__namespace.string().meta({
              description: "The name of the role to delete"
            })
          }),
          z__namespace.object({
            roleId: z__namespace.string().meta({
    ...
    async (ctx) => {
      ...
      const canDeleteRole = await hasPermission.hasPermission(
        {
          options,
          organizationId,
          permissions: {
...
const listOrgRoles = (options) => {
  ...
  return session.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const canListRoles = await hasPermission.hasPermission(
        {
          options,
          organizationId,
          permissions: {
  ...
  return session.createAuthEndpoint(
    "/organization/update-role",
    {
      ...
      body: z__namespace.object({
        ...
        data: z__namespace.object({
          permission: z__namespace.record(z__namespace.string(), z__namespace.array(z__namespace.string())).optional().meta({
...
Please refer to the documentation here: https://better-auth.com/docs/plugins/organization#dynamic-access-control`
      ...
      const canUpdateRole = await hasPermission.hasPermission(
        {
          options,
          organizationId,
          role: member.role,
  ...
  for await (const [resource, permissions] of permissionEntries) {
    for await (const perm of permissions) {
      hasNecessaryPermissions.push({
        ...
        hasPermission: await hasPermission.hasPermission(
          {
            options,
            organizationId,
...
const organization = (options) => {
  ...
  const api = shimContext(endpoints, {
    ...
    getSession: async (context) => {
      return await session.getSessionFromCtx(context);
    }
  });
  return {
    id: "organization",
    endpoints: {
      ...api,
      hasPermission: session.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const result = await hasPermission.hasPermission(
            {
              role: member.role,
              options: options || {},
