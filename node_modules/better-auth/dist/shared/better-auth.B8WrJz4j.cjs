...
const kyselyAdapter = require('./better-auth.CfoT4oLs.cjs');
const memoryAdapter = require('./better-auth.DoIzix-f.cjs');
...
async function getAdapter(options) {
  ...
  const { kysely, databaseType } = await kyselyAdapter.createKyselyAdapter(options);
  if (!kysely) {
    throw new index.BetterAuthError("Failed to initialize database adapter");
  }
...
async function getMigrations(config) {
  ...
  const logger$1 = logger.createLogger(config.logger);
