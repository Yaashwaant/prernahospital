...
const originCheckMiddleware = session.createAuthMiddleware(async (ctx) => {
  ...
  const matchesPattern = (url$1, pattern) => {
    ...
    if (pattern.includes("*")) {
      if (pattern.includes("://")) {
        return wildcardMatch(pattern)(url.getOrigin(url$1) || url$1);
      }
...
const originCheck = (getValue) => session.createAuthMiddleware(async (ctx) => {
  if (!ctx.request) {
    return;
  }
  const { context } = ctx;
...
async function createEmailVerificationToken(secret, email, updateTo, expiresIn = 3600) {
  const token = await crypto_index.signJWT(
    {
...
const sendVerificationEmail = session.createAuthEndpoint(
  "/send-verification-email",
  {
    method: "POST",
    body: z__namespace.object({
      email: z__namespace.email().meta({
  ...
  async (ctx) => {
    ...
    if (session$1?.user.emailVerified) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: "You can only send a verification email to an unverified email"
      });
    }
    if (session$1?.user.email !== email) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: "You can only send a verification email to your own email"
      });
    }
    await sendVerificationEmailFn(ctx, session$1.user);
    return ctx.json({
      status: true
    });
  }
);
const verifyEmail = session.createAuthEndpoint(
  "/verify-email",
  {
    method: "GET",
  ...
  async (ctx) => {
    ...
    const schema = z__namespace.object({
      ...
      updateTo: z__namespace.string().optional()
    });
    ...
    if (parsed.updateTo) {
      ...
      if (session$1.user.email !== parsed.email) {
        if (ctx.query.callbackURL) {
          throw ctx.redirect(`${ctx.query.callbackURL}?error=unauthorized`);
        }
        return redirectOnError("unauthorized");
    ...
    if (ctx.context.options.emailVerification?.autoSignInAfterVerification) {
      const currentSession = await session.getSessionFromCtx(ctx);
...
async function parseState(c) {
  ...
  const parsedData = z__namespace.object({
    callbackURL: z__namespace.string(),
    codeVerifier: z__namespace.string(),
    errorURL: z__namespace.string().optional(),
    newUserURL: z__namespace.string().optional(),
    expiresAt: z__namespace.number(),
    link: z__namespace.object({
      email: z__namespace.string(),
      userId: z__namespace.coerce.string()
    }).optional(),
    requestSignUp: z__namespace.boolean().optional()
...
function getOAuth2Tokens(data) {
  return {
    ...
    accessTokenExpiresAt: data.expires_in ? date.getDate(data.expires_in, "sec") : void 0,
    refreshTokenExpiresAt: data.refresh_token_expires_in ? date.getDate(data.refresh_token_expires_in, "sec") : void 0,
...
function decryptOAuthToken(token, ctx) {
  ...
  if (ctx.options.account?.encryptOAuthTokens) {
    return crypto_index.symmetricDecrypt({
      key: ctx.secret,
      data: token
    });
  }
...
function setTokenUtil(token, ctx) {
  if (ctx.options.account?.encryptOAuthTokens && token) {
    return crypto_index.symmetricEncrypt({
      key: ctx.secret,
      data: token
    });
  }
  ...
  return {
    ...
    async createAuthorizationURL({ state, scopes, codeVerifier, redirectURI }) {
      if (!options.clientId || !options.clientSecret) {
        logger.logger.error("Client Id and Secret are required for Atlassian");
        throw new index.BetterAuthError("CLIENT_ID_AND_SECRET_REQUIRED");
      }
      if (!codeVerifier) {
        throw new index.BetterAuthError("codeVerifier is required for Atlassian");
      }
    ...
    async getUserInfo(token) {
      ...
      try {
        ...
        logger.logger.error("Failed to fetch user info from Figma:", error);
        return null;
      }
    },
    options
...
const cognito = (options) => {
  if (!options.domain || !options.region || !options.userPoolId) {
    ...
    throw new index.BetterAuthError("DOMAIN_AND_REGION_REQUIRED");
  }
  ...
  return {
    ...
    async createAuthorizationURL({ state, scopes, codeVerifier, redirectURI }) {
      ...
      if (options.requireClientSecret && !options.clientSecret) {
        ...
        throw new index.BetterAuthError("CLIENT_SECRET_REQUIRED");
      }
    ...
    async verifyIdToken(token, nonce) {
      ...
      try {
        ...
        logger.logger.error("Failed to verify ID token:", error);
        return false;
      }
    },
    async getUserInfo(token) {
      ...
      if (token.idToken) {
        try {
          ...
          logger.logger.error("Failed to decode ID token:", error);
        }
      }
      if (token.accessToken) {
        try {
          ...
          logger.logger.error("Failed to fetch user info from Cognito:", error);
        }
      }
      return null;
    },
...
const getCognitoPublicKey = async (kid, region, userPoolId) => {
  ...
  try {
    ...
    logger.logger.error("Failed to fetch Cognito public key:", error);
    throw error;
  }
};
...
const figma = (options) => {
  return {
    ...
    async createAuthorizationURL({ state, scopes, codeVerifier, redirectURI }) {
      ...
      if (!codeVerifier) {
        throw new index.BetterAuthError("codeVerifier is required for Figma");
      }
    ...
    async getUserInfo(token) {
      ...
      try {
        ...
        if (!profile) {
          logger.logger.error("Failed to fetch user from Figma");
          return null;
        }
        const userMap = await options.mapProfileToUser?.(profile);
        return {
...
const salesforce = (options) => {
  ...
  return {
    ...
    async createAuthorizationURL({ state, scopes, codeVerifier, redirectURI }) {
      ...
      if (!codeVerifier) {
        throw new index.BetterAuthError("codeVerifier is required for Salesforce");
      }
    ...
    async getUserInfo(token) {
      ...
      try {
        ...
        if (!user) {
          logger.logger.error("Failed to fetch user info from Salesforce");
          return null;
        }
        const userMap = await options.mapProfileToUser?.(user);
        return {
        ...
        logger.logger.error("Failed to fetch user info from Salesforce:", error);
        return null;
      }
    },
    options
  ...
  return {
    ...
    validateAuthorizationCode: async ({ code, redirectURI }) => {
      ...
      try {
        ...
        if (!response.data) {
          throw new index.BetterAuthError("FAILED_TO_GET_ACCESS_TOKEN");
        }
        ...
        logger.logger.error("PayPal token exchange failed:", error);
        throw new index.BetterAuthError("FAILED_TO_GET_ACCESS_TOKEN");
      }
    },
    refreshAccessToken: options.refreshAccessToken ? options.refreshAccessToken : async (refreshToken) => {
      ...
      try {
        ...
        if (!response.data) {
          throw new index.BetterAuthError("FAILED_TO_REFRESH_ACCESS_TOKEN");
        }
        const data = response.data;
        return {
          accessToken: data.access_token,
        ...
        logger.logger.error("PayPal token refresh failed:", error);
        throw new index.BetterAuthError("FAILED_TO_REFRESH_ACCESS_TOKEN");
      }
    },
    async verifyIdToken(token, nonce) {
      ...
      try {
        ...
        logger.logger.error("Failed to verify PayPal ID token:", error);
        return false;
      }
    },
    async getUserInfo(token) {
      ...
      if (!token.accessToken) {
        logger.logger.error("Access token is required to fetch PayPal user info");
        return null;
      }
      try {
        ...
        if (!response.data) {
          logger.logger.error("Failed to fetch user info from PayPal");
          return null;
        }
        ...
        logger.logger.error("Failed to fetch user info from PayPal:", error);
        return null;
      }
    },
    options
...
const signInSocial = session.createAuthEndpoint(
  "/sign-in/social",
  {
    method: "POST",
    body: z__namespace.object({
       ...
       */
      newUserCallbackURL: z__namespace.string().optional(),
      /**
      ...
      idToken: z__namespace.optional(
        z__namespace.object({
           ...
           */
          nonce: z__namespace.string().meta({
            description: "Nonce used to generate the token"
          }).optional(),
          /**
           * Access token from the provider
           */
          accessToken: z__namespace.string().meta({
            description: "Access token from the provider"
          }).optional(),
          /**
           * Refresh token from the provider
           */
          refreshToken: z__namespace.string().meta({
            description: "Refresh token from the provider"
          }).optional(),
          /**
           * Expiry date of the token
           */
          expiresAt: z__namespace.number().meta({
       ...
       */
      loginHint: z__namespace.string().meta({
  ...
  async (c) => {
    ...
    if (!provider) {
      ...
      throw new betterCall.APIError("NOT_FOUND", {
        message: session.BASE_ERROR_CODES.PROVIDER_NOT_FOUND
      });
    }
    if (c.body.idToken) {
      if (!provider.verifyIdToken) {
        ...
        throw new betterCall.APIError("NOT_FOUND", {
          message: session.BASE_ERROR_CODES.ID_TOKEN_NOT_SUPPORTED
        });
      }
      ...
      if (!valid) {
        ...
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: session.BASE_ERROR_CODES.INVALID_TOKEN
        });
      }
      ...
      if (!userInfo || !userInfo?.user) {
        ...
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: session.BASE_ERROR_CODES.FAILED_TO_GET_USER_INFO
        });
      }
      if (!userInfo.user.email) {
        ...
        throw new betterCall.APIError("UNAUTHORIZED", {
          message: session.BASE_ERROR_CODES.USER_EMAIL_NOT_FOUND
        });
      }
...
const signInEmail = session.createAuthEndpoint(
  "/sign-in/email",
  {
    method: "POST",
    body: z__namespace.object({
  ...
  async (ctx) => {
    ...
    const isValidEmail = z__namespace.string().email().safeParse(email);
    ...
    if (!user) {
      ...
      throw new betterCall.APIError("UNAUTHORIZED", {
        message: session.BASE_ERROR_CODES.INVALID_EMAIL_OR_PASSWORD
      });
    }
    ...
    if (ctx.context.options?.emailAndPassword?.requireEmailVerification && !user.user.emailVerified) {
      if (!ctx.context.options?.emailVerification?.sendVerificationEmail) {
        throw new betterCall.APIError("FORBIDDEN", {
          message: session.BASE_ERROR_CODES.EMAIL_NOT_VERIFIED
        });
      }
...
const schema = z__namespace.object({
  ...
  state: z__namespace.string().optional(),
  user: z__namespace.string().optional()
});
const callbackOAuth = session.createAuthEndpoint(
  "/callback/:id",
  {
    method: ["GET", "POST"],
...
const signOut = session.createAuthEndpoint(
  "/sign-out",
  {
    method: "POST",
    requireHeaders: true,
  ...
  async (ctx) => {
    ...
    if (!sessionCookieToken) {
      ...
      throw new betterCall.APIError("BAD_REQUEST", {
        message: session.BASE_ERROR_CODES.FAILED_TO_GET_SESSION
      });
    }
...
const requestPasswordReset = session.createAuthEndpoint(
  "/request-password-reset",
  {
    ...
    body: z__namespace.object({
       ...
       * token=VALID_TOKEN
       */
      redirectTo: z__namespace.string().meta({
...
const forgetPassword = session.createAuthEndpoint(
  "/forget-password",
  {
    method: "POST",
    body: z__namespace.object({
       ...
       * The email address of the user to send a password reset email to.
       */
      email: z__namespace.string().email().meta({
        description: "The email address of the user to send a password reset email to"
      }),
      /**
       * The URL to redirect the user to reset their password.
...
const requestPasswordResetCallback = session.createAuthEndpoint(
...
const resetPassword = session.createAuthEndpoint(
  "/reset-password",
  {
    method: "POST",
    query: z__namespace.object({
...
const updateUser = () => session.createAuthEndpoint(
  "/update-user",
  {
    method: "POST",
  ...
  async (ctx) => {
    ...
    if (body.email) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: session.BASE_ERROR_CODES.EMAIL_CAN_NOT_BE_UPDATED
      });
    }
...
const changePassword = session.createAuthEndpoint(
  "/change-password",
  {
    method: "POST",
    body: z__namespace.object({
       ...
       */
      currentPassword: z__namespace.string().meta({
       ...
       */
      revokeOtherSessions: z__namespace.boolean().meta({
  ...
  async (ctx) => {
    ...
    if (!account || !account.password) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: session.BASE_ERROR_CODES.CREDENTIAL_ACCOUNT_NOT_FOUND
      });
    }
    ...
    if (revokeOtherSessions) {
      await ctx.context.internalAdapter.deleteSessions(session$1.user.id);
...
const setPassword = session.createAuthEndpoint(
  "/set-password",
  {
    method: "POST",
    body: z__namespace.object({
...
const deleteUser = session.createAuthEndpoint(
  "/delete-user",
  {
    method: "POST",
    use: [session.sensitiveSessionMiddleware],
  ...
  async (ctx) => {
    ...
    if (!ctx.body.password && ctx.context.sessionConfig.freshAge !== 0) {
      const currentAge = new Date(session$1.session.createdAt).getTime();
      const freshAge = ctx.context.sessionConfig.freshAge * 1e3;
      const now = Date.now();
      if (now - currentAge > freshAge * 1e3) {
        throw new betterCall.APIError("BAD_REQUEST", {
          message: session.BASE_ERROR_CODES.SESSION_EXPIRED
        });
      }
    }
    const beforeDelete = ctx.context.options.user.deleteUser?.beforeDelete;
    if (beforeDelete) {
      await beforeDelete(session$1.user, ctx.request);
    }
    await ctx.context.internalAdapter.deleteUser(session$1.user.id);
    await ctx.context.internalAdapter.deleteSessions(session$1.user.id);
    await ctx.context.internalAdapter.deleteAccounts(session$1.user.id);
    cookies_index.deleteSessionCookie(ctx);
    const afterDelete = ctx.context.options.user.deleteUser?.afterDelete;
    if (afterDelete) {
      await afterDelete(session$1.user, ctx.request);
    }
    return ctx.json({
      success: true,
      message: "User deleted"
...
const deleteUserCallback = session.createAuthEndpoint(
  ...
  async (ctx) => {
    ...
    if (token.value !== session$1.user.id) {
      throw new betterCall.APIError("NOT_FOUND", {
        message: session.BASE_ERROR_CODES.INVALID_TOKEN
      });
    }
...
const changeEmail = session.createAuthEndpoint(
  "/change-email",
  {
    method: "POST",
    body: z__namespace.object({
      newEmail: z__namespace.email().meta({
    ...
    if (ctx.context.session.user.emailVerified !== true) {
      ...
      if (existing) {
        throw new betterCall.APIError("UNPROCESSABLE_ENTITY", {
          message: session.BASE_ERROR_CODES.USER_ALREADY_EXISTS
        });
      }
...
const error = session.createAuthEndpoint(
  "/error",
  {
    method: "GET",
    metadata: {
...
const ok = session.createAuthEndpoint(
  "/ok",
  {
    method: "GET",
    metadata: {
...
const listUserAccounts = session.createAuthEndpoint(
  "/list-accounts",
  {
    method: "GET",
    use: [session.sessionMiddleware],
...
const linkSocialAccount = session.createAuthEndpoint(
  "/link-social",
  {
    method: "POST",
    requireHeaders: true,
    body: z__namespace.object({
       ...
       */
      idToken: z__namespace.object({
        token: z__namespace.string(),
        nonce: z__namespace.string().optional(),
        accessToken: z__namespace.string().optional(),
        refreshToken: z__namespace.string().optional(),
        scopes: z__namespace.array(z__namespace.string()).optional()
      }).optional(),
      /**
  ...
  async (c) => {
    const session$1 = c.context.session;
    const provider = c.context.socialProviders.find(
      (p) => p.id === c.body.provider
    );
    if (!provider) {
    ...
    if (c.body.idToken) {
      ...
      if (linkingUserInfo.user.email !== session$1.user.email && c.context.options.account?.accountLinking?.allowDifferentEmails !== true) {
        throw new betterCall.APIError("UNAUTHORIZED", {
      ...
      try {
        ...
        throw new betterCall.APIError("EXPECTATION_FAILED", {
          message: "Account not linked - unable to create account"
        });
      }
      if (c.context.options.account?.accountLinking?.updateUserInfoOnLink === true) {
        try {
          await c.context.internalAdapter.updateUser(session$1.user.id, {
...
const unlinkAccount = session.createAuthEndpoint(
  "/unlink-account",
  {
    method: "POST",
    body: z__namespace.object({
      providerId: z__namespace.string(),
      accountId: z__namespace.string().optional()
    }),
  ...
  async (ctx) => {
    ...
    if (accounts.length === 1 && !ctx.context.options.account?.accountLinking?.allowUnlinkingAll) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: session.BASE_ERROR_CODES.FAILED_TO_UNLINK_LAST_ACCOUNT
      });
    }
    ...
    if (!accountExist) {
      throw new betterCall.APIError("BAD_REQUEST", {
        message: session.BASE_ERROR_CODES.ACCOUNT_NOT_FOUND
      });
    }
...
const getAccessToken = session.createAuthEndpoint(
  "/get-access-token",
  {
    method: "POST",
    body: z__namespace.object({
  ...
  async (ctx) => {
    ...
    let resolvedUserId = session$1?.user?.id || userId;
    if (!resolvedUserId) {
      throw new betterCall.APIError("BAD_REQUEST", {
...
const refreshToken = session.createAuthEndpoint(
  "/refresh-token",
  {
    method: "POST",
    body: z__namespace.object({
...
const accountInfo = session.createAuthEndpoint(
  "/account-info",
  {
    method: "POST",
    use: [session.sessionMiddleware],
...
const defuReplaceArrays = defu.createDefu((obj, key, value) => {
...
function toAuthEndpoints(endpoints, ctx) {
  ...
  for (const [key, endpoint] of Object.entries(endpoints)) {
    api[key] = async (context) => {
      ...
      if (result.response instanceof betterCall.APIError && logger.shouldPublishLog(authContext.logger.level, "debug")) {
  ...
  for (const hook of hooks) {
    if (hook.matcher(context)) {
      const result = await hook.handler({
        ...
        if (e instanceof betterCall.APIError && logger.shouldPublishLog(context.context.logger.level, "debug")) {
  ...
  for (const hook of hooks) {
    if (hook.matcher(context)) {
      const result = await hook.handler(context).catch((e) => {
        if (e instanceof betterCall.APIError) {
          if (logger.shouldPublishLog(context.context.logger.level, "debug")) {
            e.stack = e.errorStack;
          }
          return {
            response: e,
...
exports.decryptOAuthToken = decryptOAuthToken;
exports.deleteUser = deleteUser;
exports.deleteUserCallback = deleteUserCallback;
exports.discord = discord;
exports.dropbox = dropbox;
exports.encodeOAuthParameter = encodeOAuthParameter;
exports.error = error;
...
exports.requestPasswordResetCallback = requestPasswordResetCallback;
...
exports.setTokenUtil = setTokenUtil;
...
exports.toAuthEndpoints = toAuthEndpoints;
