...
const admin = (options) => {
  ...
  const adminMiddleware = session.createAuthMiddleware(async (ctx) => {
  ...
  return {
    ...
    endpoints: {
       ...
       */
      setRole: session.createAuthEndpoint(
        "/admin/set-role",
        {
          method: "POST",
      ...
      getUser: session.createAuthEndpoint(
        "/admin/get-user",
        {
          method: "GET",
        ...
        async (ctx) => {
          const { id } = ctx.query;
          const canGetUser = hasPermission.hasPermission({
            userId: ctx.context.session.user.id,
            role: ctx.context.session.user.role,
            options: opts,
            permissions: {
          ...
          return schema$1.parseUserOutput(ctx.context.options, user);
        }
      ),
      /**
       * ### Endpoint
       ...
       */
      createUser: session.createAuthEndpoint(
        "/admin/create-user",
        {
          method: "POST",
          body: z__namespace.object({
      ...
      adminUpdateUser: session.createAuthEndpoint(
        ...
        async (ctx) => {
          const canUpdateUser = hasPermission.hasPermission({
            userId: ctx.context.session.user.id,
            role: ctx.context.session.user.role,
            options: opts,
            permissions: {
      ...
      listUsers: session.createAuthEndpoint(
        "/admin/list-users",
        {
          method: "GET",
          use: [adminMiddleware],
       ...
       */
      listUserSessions: session.createAuthEndpoint(
       ...
       */
      unbanUser: session.createAuthEndpoint(
        "/admin/unban-user",
        {
          method: "POST",
          body: z__namespace.object({
       ...
       */
      banUser: session.createAuthEndpoint(
        "/admin/ban-user",
        {
          method: "POST",
          body: z__namespace.object({
       ...
       */
      impersonateUser: session.createAuthEndpoint(
       ...
       */
      stopImpersonating: session.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          if (!session$1.session.impersonatedBy) {
            throw new betterCall.APIError("BAD_REQUEST", {
              message: "You are not impersonating anyone"
            });
          }
          const user = await ctx.context.internalAdapter.findUserById(
            session$1.session.impersonatedBy
          );
          if (!user) {
            throw new betterCall.APIError("INTERNAL_SERVER_ERROR", {
       ...
       */
      revokeUserSession: session.createAuthEndpoint(
       ...
       */
      revokeUserSessions: session.createAuthEndpoint(
       ...
       */
      removeUser: session.createAuthEndpoint(
        "/admin/remove-user",
        {
          method: "POST",
          body: z__namespace.object({
       ...
       */
      setUserPassword: session.createAuthEndpoint(
       ...
       */
      userHasPermission: session.createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          if (!session$1 && !ctx.body.userId && !ctx.body.role) {
            throw new betterCall.APIError("BAD_REQUEST", {
              message: "user id or role is required"
            });
          }
          const user = session$1?.user || (ctx.body.role ? { id: ctx.body.userId || "", role: ctx.body.role } : null) || await ctx.context.internalAdapter.findUserById(
            ctx.body.userId
          );
          if (!user) {
            throw new betterCall.APIError("BAD_REQUEST", {
